...
import static saf.settings.AppPropertyType.EXPORT_FILE_EXT;
import static saf.settings.AppPropertyType.EXPORT_FILE_EXT_DESC;
...
import static saf.settings.AppPropertyType.NEW_MAP_TITLE;
...
import saf.ui.NewMapDialog;
import saf.ui.Progressing;
...
public class AppFileController {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
		...
		fc.getExtensionFilters().addAll(
		new ExtensionFilter(props.getProperty(EXPORT_FILE_EXT_DESC), props.getProperty(EXPORT_FILE_EXT)));
	...
	fc.setTitle(props.getProperty(LOAD_WORK_TITLE));
		...
		AppFileComponent fileManager = app.getFileComponent();
                ...
                Progressing progress = Progressing.getSingleton();
                progress.show("Loading data","Click start to load");
    ...
    private void promptToNew() {
	// WE'LL NEED TO GET CUSTOMIZED STUFF WITH THIS
	PropertiesManager props = PropertiesManager.getPropertiesManager();
	
        // OPEN THE NEW MAP DIALOG 
        NewMapDialog newMapDialog = NewMapDialog.getSingleton();
        newMapDialog.show(props.getProperty(NEW_MAP_TITLE));
        
        // 3 ELEMENTS FROM THE USER 
        String mapName = newMapDialog.getMapName();
        String dataFile = newMapDialog.getDataFile();
        File directoryFile = newMapDialog.getDirectoryFile();
        ...
        if (selection.equals("OK")){
            try {
                AppDataComponent dataManager = app.getDataComponent();
		AppFileComponent fileManager = app.getFileComponent();
                fileManager.newData(dataManager, mapName, dataFile, directoryFile);
                
                app.getWorkspaceComponent().reloadWorkspace();
                
                FileChooser fc = new FileChooser();
		fc.setInitialDirectory(new File(directoryFile.getAbsolutePath()));
		fc.getExtensionFilters().addAll(
		new ExtensionFilter(props.getProperty(WORK_FILE_EXT_DESC), props.getProperty(WORK_FILE_EXT)));
                
                
//		File selectedFile = new File("/Users/Cathy/regiovincomapeditor/RegioVincoMapEditor/export/lll/");
